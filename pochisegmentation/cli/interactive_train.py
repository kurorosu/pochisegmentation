...
from pochisegmentation.cli.interactive import InteractiveRunner
...
def interactive_train() -> None:
    ...
    # 設定を辞書に変換
    config = interactive_config.to_dict()
    ...
    # 対話で選択した設定を Python ファイルとして保存
    config_content = interactive_config.to_python_config()
    config_path = Path(workspace_path) / "config.py"
    config_path.write_text(config_content, encoding="utf-8")
    logger.info(f"設定ファイルを保存: {config_path}")
