"""GradientTracerクラスのテスト."""

import tempfile
from pathlib import Path

import pytest
import torch
import torch.nn as nn

from pochisegmentation.visualization import GradientTracer


class SimpleModel(nn.Module):
    """テスト用の簡単なモデル."""

    def __init__(self) -> None:
        """初期化."""
        super().__init__()
        self.fc1 = nn.Linear(10, 5)
        self.fc2 = nn.Linear(5, 2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """順伝播."""
        x = torch.relu(self.fc1(x))
        return self.fc2(x)


@pytest.fixture
def model() -> SimpleModel:
    """テスト用モデルのフィクスチャ."""
    return SimpleModel()


@pytest.fixture
def gradient_tracer() -> GradientTracer:
    """GradientTracerのフィクスチャ."""
    return GradientTracer()


def test_record_gradients(model: SimpleModel, gradient_tracer: GradientTracer) -> None:
    """勾配記録のテスト."""
    # 勾配を生成
    x = torch.randn(4, 10)
    y = torch.randint(0, 2, (4,))
    output = model(x)
    loss = nn.functional.cross_entropy(output, y)
    loss.backward()

    # 勾配を記録
    gradient_tracer.record_gradients(model, epoch=1)

    # 検証
    assert len(gradient_tracer.epochs) == 1
    assert gradient_tracer.epochs[0] == 1
    # fc1.weight, fc1.bias, fc2.weight, fc2.bias
    assert len(gradient_tracer.layer_names) == 4
    assert len(gradient_tracer.gradient_history) == 4


def test_multiple_epochs(model: SimpleModel, gradient_tracer: GradientTracer) -> None:
    """複数エポックの記録テスト."""
    for epoch in range(1, 4):
        # 勾配を生成
        x = torch.randn(4, 10)
        y = torch.randint(0, 2, (4,))
        output = model(x)
        loss = nn.functional.cross_entropy(output, y)
        loss.backward()

        # 勾配を記録
        gradient_tracer.record_gradients(model, epoch=epoch)

    # 検証
    assert len(gradient_tracer.epochs) == 3
    assert gradient_tracer.epochs == [1, 2, 3]
    for layer_name in gradient_tracer.layer_names:
        assert len(gradient_tracer.gradient_history[layer_name]) == 3


def test_save_csv(model: SimpleModel, gradient_tracer: GradientTracer) -> None:
    """CSV保存のテスト."""
    # 複数エポックの勾配を記録
    for epoch in range(1, 4):
        x = torch.randn(4, 10)
        y = torch.randint(0, 2, (4,))
        output = model(x)
        loss = nn.functional.cross_entropy(output, y)
        loss.backward()
        gradient_tracer.record_gradients(model, epoch=epoch)

    # CSV保存
    with tempfile.TemporaryDirectory() as tmpdir:
        csv_path = Path(tmpdir) / "gradient_trace.csv"
        gradient_tracer.save_csv(csv_path)

        # ファイルが存在することを確認
        assert csv_path.exists()

        # CSVの内容を確認
        with open(csv_path, "r") as f:
            lines = f.readlines()
            assert len(lines) == 4  # ヘッダー + 3エポック
            assert "epoch" in lines[0]


def test_get_summary(model: SimpleModel, gradient_tracer: GradientTracer) -> None:
    """サマリー取得のテスト."""
    # 複数エポックの勾配を記録
    for epoch in range(1, 4):
        x = torch.randn(4, 10)
        y = torch.randint(0, 2, (4,))
        output = model(x)
        loss = nn.functional.cross_entropy(output, y)
        loss.backward()
        gradient_tracer.record_gradients(model, epoch=epoch)

    # サマリーを取得
    summary = gradient_tracer.get_summary()

    # 検証
    assert summary["total_epochs"] == 3
    assert summary["total_layers"] == 4
    assert len(summary["layer_names"]) == 4
    assert "layer_stats" in summary
    assert len(summary["layer_stats"]) == 4


def test_empty_gradient_tracer() -> None:
    """空のトレーサーのテスト."""
    tracer = GradientTracer()
    summary = tracer.get_summary()
    assert summary == {}


def test_no_gradient(model: SimpleModel, gradient_tracer: GradientTracer) -> None:
    """勾配がない場合のテスト."""
    # 勾配なしで記録
    gradient_tracer.record_gradients(model, epoch=1)

    # 検証（勾配がNoneの場合は0として記録される）
    assert len(gradient_tracer.epochs) == 1
    for layer_name in gradient_tracer.layer_names:
        assert gradient_tracer.gradient_history[layer_name][0] == 0.0


def test_exclude_patterns(model: SimpleModel) -> None:
    """除外パターンのテスト."""
    # bias項を除外するトレーサー
    tracer = GradientTracer(exclude_patterns=["\\.bias"])

    # 勾配を生成
    x = torch.randn(4, 10)
    y = torch.randint(0, 2, (4,))
    output = model(x)
    loss = nn.functional.cross_entropy(output, y)
    loss.backward()

    # 勾配を記録
    tracer.record_gradients(model, epoch=1)

    # 検証: bias項が除外されている（fc1.weight, fc2.weightのみ）
    assert len(tracer.layer_names) == 2
    assert "fc1.weight" in tracer.layer_names
    assert "fc2.weight" in tracer.layer_names
    assert "fc1.bias" not in tracer.layer_names
    assert "fc2.bias" not in tracer.layer_names


def test_aggregation_methods(model: SimpleModel) -> None:
    """集約方法のテスト."""
    methods = ["median", "mean", "max", "rms"]

    for method in methods:
        tracer = GradientTracer(
            group_by_block=False, aggregation_method=method  # グループ化なし
        )

        # 勾配を生成
        x = torch.randn(4, 10)
        y = torch.randint(0, 2, (4,))
        output = model(x)
        loss = nn.functional.cross_entropy(output, y)
        loss.backward()

        # 勾配を記録
        tracer.record_gradients(model, epoch=1)

        # 検証: 記録が正常に行われている
        assert len(tracer.epochs) == 1
        assert len(tracer.layer_names) == 4
        for layer_name in tracer.layer_names:
            assert tracer.gradient_history[layer_name][0] >= 0.0


def test_group_by_block() -> None:
    """ブロック単位のグループ化テスト."""

    # ResNetのような構造を模倣したモデル
    class ResNetLikeModel(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.conv1 = nn.Conv2d(3, 64, kernel_size=7)
            self.bn1 = nn.BatchNorm2d(64)
            # layer1ブロック（複数のパラメータ）
            self.layer1 = nn.Sequential(
                nn.Conv2d(64, 64, kernel_size=3, padding=1),
                nn.BatchNorm2d(64),
                nn.Conv2d(64, 64, kernel_size=3, padding=1),
                nn.BatchNorm2d(64),
            )
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            self.fc = nn.Linear(64, 10)

        def forward(self, x: torch.Tensor) -> torch.Tensor:
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.layer1(x)
            x = self.avgpool(x)
            x = torch.flatten(x, 1)
            return self.fc(x)

    model = ResNetLikeModel()

    # グループ化ありのトレーサー
    tracer = GradientTracer(exclude_patterns=["fc\\.", "\\.bias"], group_by_block=True)

    # 勾配を生成
    x = torch.randn(2, 3, 32, 32)
    y = torch.randint(0, 10, (2,))
    output = model(x)
    loss = nn.functional.cross_entropy(output, y)
    loss.backward()

    # 勾配を記録
    tracer.record_gradients(model, epoch=1)

    # 検証: layer1がグループ化されている
    assert "layer1" in tracer.layer_names
    assert "conv1.weight" in tracer.layer_names
    assert "bn1.weight" in tracer.layer_names
    # layer1.0.weight, layer1.0.bias などは個別に存在しない
    assert not any("layer1.0" in name for name in tracer.layer_names)
