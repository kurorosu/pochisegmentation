...
from pochisegmentation.logging import LoggerManager
from pochisegmentation.logging.logger_manager import LogLevel
...
class TestLoggerManager:
    ...
    def test_fallback_when_colorlog_unavailable(self, mocker: Any) -> None:
        """colorlogが利用できない場合のフォールバック機能テスト."""
        # colorlogが利用できない状態をモック
        mocker.patch(
            "pochisegmentation.logging.logger_manager.COLORLOG_AVAILABLE", False
        )
    ...
    def test_colorlog_when_available(self, mocker: Any) -> None:
        """colorlogが利用可能な場合のテスト."""
        # colorlogが利用可能な状態をモック
        mocker.patch(
            "pochisegmentation.logging.logger_manager.COLORLOG_AVAILABLE", True
        )
