...
class TestUnetModel:
    ...
    def test_init_default(self) -> None:
        """デフォルトパラメータでの初期化テスト."""
        model = UnetModel(num_classes=4)
        assert model is not None

    def test_init_custom_encoder(self) -> None:
        """カスタムエンコーダーでの初期化テスト."""
        model = UnetModel(encoder_name="resnet50", num_classes=4)
        assert model is not None

    def test_init_without_pretrained(self) -> None:
        """事前学習なしでの初期化テスト."""
        model = UnetModel(num_classes=4, pretrained=False)
        assert model is not None
    ...
    def test_forward_different_sizes(self) -> None:
        """異なる入力サイズでの順伝播テスト."""
        model = UnetModel(num_classes=2, pretrained=False)
        model.eval()
    ...
    def test_custom_in_channels(self) -> None:
        """カスタム入力チャンネル数テスト."""
        model = UnetModel(num_classes=4, in_channels=1, pretrained=False)
        model.eval()
